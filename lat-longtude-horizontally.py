...
def pixel_to_latlon(x, y, width, height, bounds):
    ...
    # Flip latitude mapping to correctly orient the plots
    lat = bounds["bottom_right"][0] + (1 - (y / height)) * (bounds["top_left"][0] - bounds["bottom_right"][0])
    return lon, lat
...
plt.title('Traced Plot Shape Lines with Correct Orientation (Latitude and Longitude)')
